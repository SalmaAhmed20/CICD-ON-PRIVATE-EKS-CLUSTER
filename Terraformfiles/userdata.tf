data "template_cloudinit_config" "usrdata-script" {
  part {
    ...
    content      = <<-EOF
      ...
      echo 'aws_access_key_id = ${data.aws_ssm_parameter.access_key.value}' >> /home/ubuntu/.aws/credentials
      echo 'aws_secret_access_key = ${data.aws_ssm_parameter.secret_key.value}' >> /home/ubuntu/.aws/credentials
      ...
      docker login -u ${data.aws_ssm_parameter.dockeruser.value} -p ${data.aws_ssm_parameter.dockerpass.value}
    EOF
  }
}